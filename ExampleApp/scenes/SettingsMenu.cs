...
public class SettingsMenu : Scene
{
    ...
    public SettingsMenu(string _name = "SettingsMenu") : base(_name)
	{   
        name = _name;
        Visible = false;
		...
		//banner.Position = new Vec2i((Console.WindowWidth / 2) - banner.Display.Length / 2, 5);
		banner.center_x();
		add_child(banner);

        play.Position = new Vec2i((Console.WindowWidth / 2) - play.Display.Length / 2,Console.WindowHeight / 2);
		play.ZIndex = 100;
		...
		{
			Game.Beep(1337, 500);
            Game.CurrentScene = new Scene("empty");
        };
